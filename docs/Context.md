...
Declared `IdentityInt64` claims must be canonical signed base-10 text. Declared `IdentityInt64s` claims are bounded JSON integer arrays; context construction validates the whole array, removes duplicates, and sorts it. Non-canonical, malformed, overflowing, or oversized values are discarded and therefore fail closed. Application SQL receives typed bound integers, never interpolated claim text.
