...
func GenerateRBACGates(actionsDir, policiesDir, httpImport string) (map[string][]byte, error) {
	...
	for modelName, set := range actionSets {
		...
		for _, action := range set.Actions {
			...
			src, err := GenerateRBACGate(toSnakeCase(action.Name), grant.AllowedRoles, grant.ManagesRoles, sourcePolicy, modelName, httpImport)
			if err != nil {
	...
	if _, err := os.Stat(assignGateUser); os.IsNotExist(err) {
		...
		src, err := GenerateAssignRoleGate("role", httpImport)
		if err != nil {
...
func CanAssign(ctx *pickle.Context, model interface{ OwnerID() string }) *uuid.UUID {
	if ctx.IsAdmin() {
		id := uuid.New()
		return &id
	}
