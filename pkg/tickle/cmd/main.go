...
var templates = []struct {
	...
	perFile bool // emit one constant per source file instead of merging
}{
	{
		srcDir: "pkg/cooked",
		output: "pkg/generator/embed_http.go",
		skip:   map[string]bool{"query.go": true, "query_immutable.go": true, "scopes.go": true, "config.go": true, "transaction.go": true, "errors.go": true, "locks.go": true, "integrity.go": true, "merkle.go": true, "graphql.go": true, "scheduler.go": true},
	},
	{
		srcDir: "pkg/cooked",
		output: "pkg/generator/embed_query.go",
		only:   map[string]bool{"query.go": true, "query_immutable.go": true, "transaction.go": true, "errors.go": true, "locks.go": true, "integrity.go": true, "merkle.go": true},
	},
	{
		srcDir: "pkg/cooked",
	...
	{
		srcDir: "pkg/cooked",
		output: "pkg/generator/embed_scheduler.go",
		only:   map[string]bool{"scheduler.go": true},
	},
	{
		srcDir: "pkg/schema",
	...
	{
		srcDir:  "pkg/cooked/auth/jwt/migrations",
...
func main() {
	...
	for _, t := range templates {
		...
		if t.perFile {
			...
			if err := writePerFileEmbeds(outputPath, files); err != nil {
				fmt.Fprintf(os.Stderr, "tickle: %v\n", err)
				os.Exit(1)
			}
...
// writePerFileEmbeds writes one Go constant per source file into a single output file.
// Each constant is named from the source filename, e.g. "create_users_table.go" → embedCreateUsersTable.
func writePerFileEmbeds(path string, files map[string]string) error {
	names := sortedFileKeys(files)
	...
	for _, name := range names {
		content := files[name]
		// Strip build constraints and replace package line with placeholder
		content = replacePackageLine(content, placeholder)
		// Prepend generated header
		content = "// Code generated by Pickle. DO NOT EDIT.\n" + content
		varName := "embed_" + strings.TrimSuffix(name, ".go")
...
func replacePackageLine(src, pkg string) string {
	lines := strings.Split(src, "\n")
	var out []string
	for _, line := range lines {
		trimmed := strings.TrimSpace(line)
		if strings.HasPrefix(trimmed, "//go:build") {
			continue // strip build constraints
		}
