...
│   │   ├── graphql_*.go           ← GraphQL suite: crud, dataloader, handler, input,
│   │   │                             mutation, resolver, schema, types generators
│   │   ├── auth_generator.go      ← Generates auth driver migrations (JWT, Session, OAuth)
│   │   ├── config_generator.go    ← Generates configuration scaffolding
│   │   ├── command_generator.go   ← Generates cron/scheduled job handlers
│   │   ├── job_generator.go       ← Generates background job framework
│   │   ├── view_generator.go      ← Generates database view migrations
│   │   ├── search_generator.go    ← Generates full-text search integrations
│   │   ├── registry_generator.go  ← Generates service registry
│   │   ├── response_generator.go  ← Generates response helper functions
...
│   │   ├── query_immutable.go     ← Specialized query builder for immutable tables
│   │   ├── scopes.go              ← Scope templates (pickle:scope directives)
│   │   ├── connection.go          ← Database connection management
│   │   ├── transaction.go         ← Transaction lifecycle management
│   │   ├── locks.go               ← Database row locking primitives
│   │   ├── encryption.go          ← AES-SIV and AES-GCM for encrypted columns
│   │   ├── integrity.go           ← Merkle tree integrity for immutable audit logs
│   │   ├── merkle.go              ← Merkle tree implementation
│   │   ├── ratelimit.go           ← Per-IP and auth-aware rate limiting
│   │   ├── scheduler.go           ← Cron/scheduled job executor
│   │   ├── command.go             ← CLI command execution
│   │   ├── config.go              ← Runtime configuration
│   │   ├── reload.go              ← Live configuration reload
│   │   ├── graphql.go             ← GraphQL runtime support
...
│   │   └── auth/                  ← Auth drivers: jwt/, oauth/, session/
...
│   ├── cron-test/                 ← Test app: cron/scheduled jobs
│   ├── encryption-test/           ← Test app: encrypted/sealed columns
│   ├── ledger/                    ← Test app: immutable audit trails
│   ├── monorepo/                  ← Test app: multi-project structure
│   └── zero-graphql/              ← Test app: GraphQL schema generation
├── go.mod
└── CLAUDE.md
```
