...
func TestGenerateCommandsGlueWithSeeders(t *testing.T) {
	...
	text := string(out)
	for _, want := range []string{"type dbSeedCommand struct", "seeders.Resolve(scenario)", "bcrypt.GenerateFromPassword", "migrations.SeedExecutor", "definition.Policy", "dbSeedCommand{}"} {
